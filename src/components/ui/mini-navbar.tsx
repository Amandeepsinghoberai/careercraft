...
const AnimatedNavLink = ({ to, children }: { to: string; children: React.ReactNode }) => {
  ...
  return (
    <Link to={to} className={`group relative inline-block overflow-hidden ${textSizeClass}`}>
      <div className="flex flex-col transition-transform duration-300 ease-out transform group-hover:-translate-y-6">
...
export function Navbar() {
  ...
  const signupButtonElement = (
    <div className="relative group w-full sm:w-auto">
       ...
       <Link to="/signup" className="relative z-10 px-4 py-2 sm:px-3 text-xs sm:text-sm font-semibold text-black bg-gradient-to-br from-gray-100 to-gray-300 rounded-full hover:from-gray-200 hover:to-gray-400 transition-all duration-200 w-full sm:w-auto text-center block">
         Signup
       </Link>
    </div>
  );
